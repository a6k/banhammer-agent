...
def test_load_config_with_api_section(tmp_path):
    ...
    assert config["api"]["api_key"] == "bh_testapikey1234567890abcdef12"
...
def test_load_config_env_var_overrides_api_key(tmp_path, monkeypatch):
    ...
    config_file.write_text(textwrap.dedent("""\
        [api]
        api_key = "bh_from_file_1234567890abcdef12"
    """))
    monkeypatch.setenv("BANHAMMER_API_KEY", "bh_from_env_1234567890abcdef12")
    config = load_config(str(config_file))
    assert config["api"]["api_key"] == "bh_from_env_1234567890abcdef12"
